...
/obj/item/organ/external/proc/inspect(mob/user)
	...
	if(!do_mob(user,owner, 10))
		to_chat(user, "<span class='notice'>You must stand still to inspect [src] for wounds.</span>")
	else if(wounds.len)
	...
	if(!do_mob(user, owner, 10))
		...
		if(owner.getToxLoss() >= 40)
			to_chat(user, "<span class='warning'>[owner] has an unhealthy skin discoloration.</span>")
			bad = 1
		if(owner.getOxyLoss() >= 20)
			to_chat(user, "<span class='warning'>[owner]'s skin is unusaly pale.</span>")
			bad = 1
		if(owner.shock_stage >= 30)
			to_chat(user, "<span class='warning'>[owner]'s skin is clammy and cold.</span>")
			bad = 1
		if(status & ORGAN_DEAD)
			to_chat(user, "<span class='warning'>[src] is decaying!</span>")
			bad = 1
		if(!bad)
